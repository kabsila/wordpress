...
if (function_exists('add_image_size')) {
	add_image_size('project-thumb', 440, 440, true);
	add_image_size('project-thumb-half', 555, 370, true);
	add_image_size('project-thumb-full', 1160, 615, true);
	add_image_size('slider-thumb', 710, 9999);
	add_image_size('full-thumb', 1030, 580, true);
	add_image_size('full-slider-thumb', 1030, 9999);
}
...
function sidebarDefaultText() {
	...
	echo '" title="' . __('The latest comments to all posts in RSS', 'cmsmasters') . '">' . __('Comments', 'cmsmasters') . ' ' . 
				...
				'</li>' . "\n\t" . 
				'<li>' . 
					'<a href="http://wordpress.org/" title="' . __('S H A RE D  ON   W  P  L  O  C  K  E R .C O M ', 'cmsmasters') . '">WordPress.org</a>' . 
				'</li>' . "\r" . 
			'</ul>' . "\n" . 
		'</aside>' . "\n" . 
	'</div>' . "\n";
...
function cmsms_header_styles() {
	...
	echo '#header > .header_inner { ' . 
		...
		'#header nav { ' . 
			...
			'right : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_nav_right'] . 'px; ; ' . 
		'} ' . 
		'#header .wrap_social_icons { ' . 
			'top : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_social_top'] . 'px; ' . 
			'right : ' . $cmsms_option[CMSMS_SHORTNAME . '_header_social_right'] . 'px; ' . 
		'} ' . 
		'#header .header_html { ' . 
